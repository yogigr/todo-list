<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('todo.store') }}">
            ...
            <x-text-input id="todo" name="todo" type="text" class="mt-1 block w-full" :value="old('todo')" autofocus autocomplete="todo" />
                        ...
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                ...
                                <form method="POST" action="{{ route('todo.destroy', $todo) }}">
                                    @csrf
                                    @method('delete')
                                    <x-dropdown-link :href="route('todo.destroy', $todo)" onclick="event.preventDefault(); this.closest('form').submit();">
                                        {{ __('Delete') }}
                                    </x-dropdown-link>
                                </form>
                            </x-slot>
